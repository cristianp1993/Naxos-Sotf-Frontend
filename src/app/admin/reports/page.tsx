...
import { reportsService, type CashFlowReport } from '@/services/reportsService';
...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Report Preview - Solo para flujo de caja */}
      {showPreview && reportData && selectedReport.id === 'cash-flow' && (
        <div className="space-y-6">
          {/* Summary Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-bold">
                    {formatCurrency(reportData.resumen.total_ingresos)}
                  </p>
                </div>
            ...
            <div className="bg-gradient-to-r from-red-600 to-orange-600 rounded-2xl p-4 text-white">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-bold">
                    {formatCurrency(reportData.resumen.total_egresos)}
                  </p>
                </div>
                <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className={`bg-gradient-to-r ${reportData.resumen.resultado === 'POSITIVO' ? 'from-green-600 to-emerald-600' : 'from-red-600 to-orange-600'} rounded-2xl p-4 text-white`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white/80 text-xs sm:text-sm">Diferencia</p>
                  <p className="text-xl sm:text-2xl font-bold">
                    {formatCurrency(reportData.resumen.diferencia)}
                  </p>
                  <p className={`text-xs font-medium ${getResultColor(reportData.resumen.resultado)}`}>
                    {reportData.resumen.resultado}
                  </p>
                </div>
                <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-2xl p-4 text-white">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-bold">
                    {reportData.ventas.total_ventas}
                  </p>
                </div>
                <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-green-500/20 border border-green-400/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white text-xs bg-green-500/30 px-2 py-1 rounded-full">
                    {reportData.ventas.desglose_pagos.efectivo.cantidad} ventas
                  </span>
                </div>
                <p className="text-white text-lg font-bold">
                  {formatCurrency(reportData.ventas.desglose_pagos.efectivo.total)}
                </p>
              </div>
              ...
              <div className="bg-blue-500/20 border border-blue-400/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white text-xs bg-blue-500/30 px-2 py-1 rounded-full">
                    {reportData.ventas.desglose_pagos.transferencia.cantidad} ventas
                  </span>
                </div>
                <p className="text-white text-lg font-bold">
                  {formatCurrency(reportData.ventas.desglose_pagos.transferencia.total)}
                </p>
              </div>
              ...
              <div className="bg-purple-500/20 border border-purple-400/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white text-xs bg-purple-500/30 px-2 py-1 rounded-full">
                    {reportData.ventas.desglose_pagos.tarjeta.cantidad} ventas
                  </span>
                </div>
                <p className="text-white text-lg font-bold">
                  {formatCurrency(reportData.ventas.desglose_pagos.tarjeta.total)}
                </p>
              </div>
              ...
              <div className="bg-gray-500/20 border border-gray-400/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white text-xs bg-gray-500/30 px-2 py-1 rounded-full">
                    {reportData.ventas.desglose_pagos.otro.cantidad} ventas
                  </span>
                </div>
                <p className="text-white text-lg font-bold">
                  {formatCurrency(reportData.ventas.desglose_pagos.otro.total)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/10">
                    {reportData.ventas_por_tamaño.map((size, index) => (
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/10">
                    {reportData.gastos.detalle.map((expense, index) => (
                      <tr key={index} className="hover:bg-white/5 transition-colors">
                        <td className="px-4 py-3 whitespace-nowrap">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                ...
                <p className="text-purple-200 text-sm">
                  {formatDate(reportData.periodo.start_date)} - {formatDate(reportData.periodo.end_date)}
                </p>
              </div>
              <div className={`px-4 py-2 rounded-xl border ${getResultBgColor(reportData.resumen.resultado)}`}>
                <p className={`text-sm font-medium ${getResultColor(reportData.resumen.resultado)}`}>
                  Resultado: {reportData.resumen.resultado}
                </p>
              </div>
            </div>
          </div>
